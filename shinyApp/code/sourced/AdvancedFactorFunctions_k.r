...
# adding Crawford-Ferguson
cat("\nCFT Loadings\n-------------------------\n")
print.FLS(x$CFT,num.digits=num.digits,cutoff=cutoff)
cat("\nCFQ Loadings\n-------------------------\n")
...
#A = list(F=A,Phi=NULL) # Koval: moved this to the end of CFQ section
cat(".")
# Crawford-Ferguson
...
A = list(F=A,Phi=NULL) # line moved from 813
cat(".")
#
class(A.promax)="FLS"
class(A)="FLS"
...
QuickEFAtoCFA <- function(R,n.factors,n.obs,rotation="Varimax",model.name="model.0",cutoff=0.30,alpha=0.05,make.start.values=TRUE,cov.matrix=TRUE,num.digits=4,promax.m=3){
  rotation.list <- c("Varimax","Promax","Quartimin","Bifactor", "BifactorOblique","CFT","CFQ")
  rot <- 0
  for(i in 1:5)
